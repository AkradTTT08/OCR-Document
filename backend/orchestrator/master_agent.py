...
def qa_research(project_id: str, question: str) -> str:
    """Asks the QA Research assistant a question about a specific project."""
...
class MasterAgent:
    ...
    def run(chat_history: List[Dict[str, str]]) -> str:
        """Runs the Master Agent using Gemini function calling."""
...
If the user asks for a project-specific task but doesn't specify the project ID, use get_projects() to find it.
Always respond in Thai."""
        ...
        try:
            ...
            active_prompt = contents.pop() if contents else types.Content(
            ...
            return response.text or "(ไม่มีคำตอบ)"
